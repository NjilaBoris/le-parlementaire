  ...
  image: z
    ...
    .refine((url) => {
      try {
        ...
        const hasValidExtension = IMAGE_EXTENSIONS.test(parsed.pathname);
        const isTrustedHost = TRUSTED_IMAGE_HOSTS.includes(parsed.hostname);

        return hasValidExtension || isTrustedHost;
      } catch {
        return false;
      }
    }, "URL must contain a valid image extension or come from a trusted image source"),
});
